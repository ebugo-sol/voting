...
contract Ballot {
    ...
    function giverRightToVoters(address[] calldata votersArray) external {
        require(
            msg.sender == chairperson,
            "Only chairperson can give right to vote."
        );
    ...
    function delegate(address to) external {
        ...
        require(
            delegate_.weight >= 1,
            "Selected delegate has no voting rights."
        );
        ...
        returns (uint[] memory winningProposals_)
    {
        ...
        for (uint i = 0; i < proposals.length; i++) {
            if (proposals[i].voteCount > winningVoteCount) {
                winningVoteCount = proposals[i].voteCount;
                winningProposalsCount = 1;
            } else {
                winningProposalsCount++;
            }
        }

        winningProposals_ = new uint[](winningProposalsCount);
        uint index = 0;
        for (uint i = 0; i < proposals.length; i++) {
            if (proposals[i].voteCount == winningVoteCount) {
                winningProposals_[index] = i;
                index++;
            }
        }
    }
